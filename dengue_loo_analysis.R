...
ps_mp_dataS1 <- list(
  ...
  X = dfS1$time,
  P = t(matrix(
    data = c(dfS1$sero1, dfS1$sero2, dfS1$sero3, dfS1$sero4),
    ncol = 4
  )),
  week_effect = 1,
  DOW = (dfS1$time %% 1) + 1,
...
ps_mp_dataS2 <- list(
  ...
  X = dfS2$time,
  P = t(matrix(
    data = c(dfS2$sero1, dfS2$sero2, dfS2$sero3, dfS2$sero4),
    ncol = 4
  )),
  week_effect = 1,
  DOW = (dfS2$time %% 1) + 1,
...
mod_incB <- ps_incidence(
  ps_mp_fitB,
  df2$time,
  num_days = nrow(df2),
  time_labels = df2$date,
  num_path = 4,
  days_per_knot = 5,
  pathogen_names = c("1", "2", "3", "4")
)

mod_inc1 <- ps_incidence(
  ps_mp_fitS1,
  dfS1$time,
  num_days = nrow(dfS1),
  time_labels = dfS1$date,
  num_path = 4,
  days_per_knot = 5,
  pathogen_names = c("1", "2", "3", "4")
)

mod_inc2 <- ps_incidence(
  ps_mp_fitS2,
  dfS2$time,
  num_days = nrow(dfS2),
  time_labels = dfS2$date,
  num_path = 4,
  days_per_knot = 5,
  pathogen_names = c("1", "2", "3", "4")
)
