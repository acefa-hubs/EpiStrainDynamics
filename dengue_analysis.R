...
ps_mp_dataS <- list(num_data = nrow(dfS),
                   num_path = 4,
                   ...
                   Y = dfS$cases,
                   X = dfS$time,
                   P = t(matrix(data= c(dfS$sero1, dfS$sero2, dfS$sero3, dfS$sero4), ncol=4)),
                   week_effect = 1,
                   DOW = (dfS$time %% 1)+1,
                   cov_structure = 1,
                   tau_mn = tau_mn,
...
ps_mp_fitS <- sampling(ps_mp_modS,
                      iter= 5000,
                      warmup = 1000,
                      chains=4,
                      data = ps_mp_dataS)
...
mod_inc <- ps_incidence(ps_mp_fit, df$time, num_days=nrow(df), time_labels = df$date,
                        num_path = 4,
                        days_per_knot = 5,
                        pathogen_names = c("1", "2", "3", "4"))

# Get time-varying growth rates
mod_gr <- ps_growth_rate(ps_mp_fit, df$time, num_days=nrow(df), time_labels = df$date,
                         num_path = 4,
                         days_per_knot = 5,
                         pathogen_names = c("1", "2", "3", "4"))
...
for(i in 1:length( pathogens )){
  ...
  mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$mask <- 1
  mod_inc[index,]$mask <- 1
  print(pathogens[i])
  #mod_gr<-mod_gr[ (!(mod_gr$pathogen==pathogens[i])) | (mod_gr$time>min_date & mod_gr$time<max_date) ,]
  grp <- 1
  for(j in 1:(length(mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$mask)-1) ){
    
    mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$group[j] <- grp
    
    if (mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$time[j+1] - mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$time[j] !=1 ){
      grp <- grp+1
  ...
  mod_gr[mod_gr$pathogen==pathogens[i] & mod_gr$time %in% dates,]$group[j+1] <- grp
    
  
}


# Dividing modelled values after 2014-07-01 by 10 - this is only for plotting purposes (second axes is added)
mod_inc$cat <- 0
mod_inc[mod_inc$time>=as.Date("2014-07-01"),]$cat<-1
mod_inc_tmp <- mod_inc
mod_inc_tmp[mod_inc_tmp$cat==1,]$y <- mod_inc_tmp[mod_inc_tmp$cat==1,]$y/10
mod_inc_tmp[mod_inc_tmp$cat==1,]$lb_50 <- mod_inc_tmp[mod_inc_tmp$cat==1,]$lb_50/10
mod_inc_tmp[mod_inc_tmp$cat==1,]$ub_50 <- mod_inc_tmp[mod_inc_tmp$cat==1,]$ub_50/10
mod_inc_tmp[mod_inc_tmp$cat==1,]$lb_95 <- mod_inc_tmp[mod_inc_tmp$cat==1,]$lb_95/10
mod_inc_tmp[mod_inc_tmp$cat==1,]$ub_95 <- mod_inc_tmp[mod_inc_tmp$cat==1,]$ub_95/10

df$cases_tmp <- df$cases
df[df$date>=as.Date("2014-07-01"),]$cases_tmp <-df[df$date>=as.Date("2014-07-01"),]$cases/10
...
plt1 <- ggplot(mod_inc_tmp[mod_inc_tmp$pathogen=="Total",])+
  ...
  annotate('text', x=as.Date("2014-07-01"),y=60, label="600", hjust=1.4)+
  annotate('text', x=as.Date("2014-07-01"),y=40, label="400", hjust=1.4)+
  annotate('text', x=as.Date("2014-07-01"),y=20, label="200", hjust=1.4)+
  geom_segment(aes(x=as.Date("2014-07-01")-20, xend=as.Date("2014-07-01"), yend=60, y=60))+
  geom_segment(aes(x=as.Date("2014-07-01")-20, xend=as.Date("2014-07-01"), yend=40, y=40))+
  geom_segment(aes(x=as.Date("2014-07-01")-20, xend=as.Date("2014-07-01"), yend=20, y=20))+
...
plt3<-ggplot(mod_gr[mod_gr$mask==1,])+
  geom_line(aes(x=time, y=y, color=pathogen, group=interaction(pathogen,group) ))+
  geom_ribbon(aes(x=time, y=y, ymin=lb_50, ymax=ub_50, fill=pathogen, group=interaction(pathogen,group) ), alpha=0.2)+
  geom_ribbon(aes(x=time, y=y, ymin=lb_95, ymax=ub_95, fill=pathogen, group=interaction(pathogen,group) ), alpha=0.2)+
  theme_bw(base_size = 14)+
  scale_color_brewer("Serotype",palette="Dark2")+
  scale_fill_brewer("Serotype",palette="Dark2")+
...
# Get incidence curves
mod_incS <- ps_incidence(ps_mp_fitS, dfS$time, num_days=nrow(dfS), time_labels = dfS$date,
                        num_path = 4,
                        days_per_knot = 5,
                        pathogen_names = c("1", "2", "3", "4"))

# Get time-varying growth rates
mod_grS <- ps_growth_rate(ps_mp_fitS, dfS$time, num_days=nrow(dfS), time_labels = dfS$date,
                         num_path = 4,
                         days_per_knot = 5,
                         pathogen_names = c("1", "2", "3", "4"))
...
for(i in 1:length( pathogens )){
  ...
  if(TRUE %in% index){
    mod_grS[mod_grS$pathogen==pathogens[i] & mod_grS$time %in% dates,]$mask <- 1
...
plt1<-ggplot(mod_incS)+
  ...
  theme(legend.position=c(0.8,0.8),
        legend.background = element_rect(color="black"))+
  coord_cartesian(xlim=c(as.Date("2023-04-01")+12,as.Date("2024-04-01")-12 )  )
...
ps_get_a <- function(post,
                     X,
                     num_days = length(X), time_labels, min_time, max_time,
...
ps_cumulative_incidence <- function(a,
  ...
  for(j in 1:num_path){
    ...
    row <- data.frame(time_min = min_time,
                      time_max = max_time,
                      y=quan[[1]],
                      lb_50 = quan[[3]],
                      ub_50 = quan[[4]],
...
raw_cumulative_incidence <- function(df, min_time, max_time){
  ...
  df_r <- data.frame()
  for(i in 1:4){
    row_df <- data.frame(time_min = min_time,
                         time_max = max_time,
                         y=mp[i,1],
                         lb_95 = mp[i,2],
                         ub_95 = mp[i,3],
...
post <- rstan::extract(ps_mp_fit)
a <- ps_get_a(post, df$time, num_days=nrow(df), time_labels = df$date,
              min_time=as.Date("2006-04-01"), max_time=as.Date("2007-03-31"),
              num_path = 4,
              days_per_knot = 5,
              pathogen_names = c("1", "2", "3", "4"))

postS <- rstan::extract(ps_mp_fitS)
aS <- ps_get_a(postS, dfS$time, num_days=nrow(dfS), time_labels = dfS$date,
               min_time=as.Date("2006-04-01"), max_time=as.Date("2007-03-31"),
               num_path = 4,
               days_per_knot = 5,
               pathogen_names = c("1", "2", "3", "4"))
...
cum_inc1 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2006-04-01"), max_time=as.Date("2007-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))

cum_inc2 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2007-04-01"), max_time=as.Date("2008-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc3 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2008-04-01"), max_time=as.Date("2009-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc4 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2009-04-01"), max_time=as.Date("2010-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc5 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2010-04-01"), max_time=as.Date("2011-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc6 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2011-04-01"), max_time=as.Date("2012-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc7 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2012-04-01"), max_time=as.Date("2013-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc8 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2013-04-01"), max_time=as.Date("2014-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc9 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2014-04-01"), max_time=as.Date("2015-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))
cum_inc10 <- ps_cumulative_incidence(a, df$time, num_days=nrow(df), time_labels = df$date,
                                    min_time=as.Date("2015-04-01"), max_time=as.Date("2016-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))

cum_inc11 <- ps_cumulative_incidence(aS, dfS$time, num_days=nrow(dfS), time_labels = dfS$date,
                                    min_time=as.Date("2023-04-01"), max_time=as.Date("2024-03-31"),
                                    num_path = 4,
                                    days_per_knot = 5,
                                    pathogen_names = c("1", "2", "3", "4"))

# Extract estimate of cumulative incidence by serotype for each season using raw data instead
raw_inc1 <- raw_cumulative_incidence(df, min_time=as.Date("2006-04-01"), max_time=as.Date("2007-03-31"))
raw_inc2 <- raw_cumulative_incidence(df, min_time=as.Date("2007-04-01"), max_time=as.Date("2008-03-31"))
raw_inc3 <- raw_cumulative_incidence(df, min_time=as.Date("2008-04-01"), max_time=as.Date("2009-03-31"))
raw_inc4 <- raw_cumulative_incidence(df, min_time=as.Date("2009-04-01"), max_time=as.Date("2010-03-31"))
raw_inc5 <- raw_cumulative_incidence(df, min_time=as.Date("2010-04-01"), max_time=as.Date("2011-03-31"))
raw_inc6 <- raw_cumulative_incidence(df, min_time=as.Date("2011-04-01"), max_time=as.Date("2012-03-31"))
raw_inc7 <- raw_cumulative_incidence(df, min_time=as.Date("2012-04-01"), max_time=as.Date("2013-03-31"))
raw_inc8 <- raw_cumulative_incidence(df, min_time=as.Date("2013-04-01"), max_time=as.Date("2014-03-31"))
raw_inc9 <- raw_cumulative_incidence(df, min_time=as.Date("2014-04-01"), max_time=as.Date("2015-03-31"))
raw_inc10 <- raw_cumulative_incidence(df, min_time=as.Date("2015-04-01"), max_time=as.Date("2016-03-31"))
raw_inc11 <- raw_cumulative_incidence(dfS, min_time=as.Date("2023-04-01"), max_time=as.Date("2024-03-31"))
...
cum_inc <- rbind(cum_inc1, cum_inc2, cum_inc3, cum_inc4, cum_inc5,
                 cum_inc6, cum_inc7, cum_inc8, cum_inc9, cum_inc10,
                 cum_inc11)
...
raw_inc <- rbind(raw_inc1, raw_inc2, raw_inc3, raw_inc4, raw_inc5,
                 raw_inc6, raw_inc7, raw_inc8, raw_inc9, raw_inc10,
                 raw_inc11)


ggplot(cum_inc[cum_inc$label=="proportion",])+
  geom_point(aes(x=year, y=y, color=pathogen), position=position_dodge(width = .5))+
  geom_errorbar(aes(x=year, y=y, ymin=lb_95, ymax=ub_95, color=pathogen), position=position_dodge(width = .5), width=0)+
  scale_color_brewer("Serotype",palette = "Dark2")+
...
test <- rbind(cum_inc[cum_inc$label=="proportion",], raw_inc[raw_inc$label=="proportion",])


ggplot(test)+
  geom_point(aes(x=year, y=y, color=pathogen, shape=method), position=position_dodge(width = .5))+
  geom_errorbar(aes(x=year, y=y, ymin=lb_95, ymax=ub_95, color=pathogen, shape=method, linetype=method), position=position_dodge(width = .5), width=0)+
  scale_color_brewer("Serotype",palette = "Dark2")+
...
raw1<-ggplot(data=df_new[df_new$date>=as.Date("2006-04-01") &df_new$date <as.Date("2016-04-01"),])+
  geom_col(aes(x=date, y=value, fill=name))+
  theme_bw(base_size = 14)+
  coord_cartesian(xlim=c(as.Date("2006-04-01")+70, as.Date("2016-04-01")-70 ))+
  scale_x_date(date_breaks = "1 year", date_labels =  "%Y")+
  ylab("Modelled cases")+
  xlab("Date")+
  scale_fill_manual("Serotype",values=cols)+
...
raw2<-ggplot(data=df_new[df_new$date>=as.Date("2023-04-01") &df_new$date <as.Date("2024-04-01"),])+
  geom_col(aes(x=date, y=value, fill=name))+
  theme_bw(base_size = 14)+
  coord_cartesian(xlim=c(as.Date("2023-04-01")+10, as.Date("2024-04-01")-10))+
...
raw3 <- ggplot(data=df_week[df_week$Week>=as.Date("2006-04-01") &df_week$Week <as.Date("2016-04-01"),])+
...
raw4<-ggplot(data=df_week[df_week$Week>=as.Date("2023-04-01") &df_week$Week <as.Date("2024-04-01"),])+
  geom_col(aes(x=Week, y=value, fill=name))+
  theme_bw(base_size = 14)+
  coord_cartesian(xlim=c(as.Date("2023-04-01")+10, as.Date("2024-04-01")-10))+
  scale_x_date(date_breaks = "4 month", date_labels =  "%b\n%Y")+
...
ggsave("Figures/Dengue_RAW.png", width=12, height=8)
