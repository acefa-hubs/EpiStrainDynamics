...
noise_comparison <- data.frame(
  ...
  correlation = vapply(noise_results, function(x) cor(x, inc_original_total), 
                       FUN.VALUE = numeric(1)),
  rmse = vapply(noise_results, function(x) sqrt(mean((x - inc_original_total)^2)), 
                FUN.VALUE = numeric(1)),
  relative_rmse = vapply(noise_results, function(x) {
    sqrt(mean((x - inc_original_total)^2)) / mean(inc_original_total)
  }, FUN.VALUE = numeric(1)),
  mean_difference = vapply(noise_results, function(x) mean(abs(x - inc_original_total)), 
                           FUN.VALUE = numeric(1))
)
