...
lineages_considered <- c("B.1.177","B.1.1.7", "B.1.617.2","BA.1","BA.2","BA.2.75","BA.4","BA.5","BQ.1","XBB")
df1S$major_lineage <- factor(df1S$major_lineage, levels=lineages_considered)

df1S$collection_date <- as.Date(df1S$collection_date)

uniq_dates <- seq.Date(min(df1S$collection_date), max(df1S$collection_date), by=1)
uniq_dates <- seq.Date(min_date, max_date, by=1)


mat <- matrix(data=NA, nrow=length(uniq_dates), ncol = length(lineages_considered)+1)
...
ps_mp_data <- list(num_data = nrow(df),
                   ...
                   Y = df$cases,
                   X = df$t,
                   P = t(df[3:13]),# Might have to fix this line
                   week_effect = 7,
                   DOW = (df$t %% 7)+1,
                   cov_structure = 0,
                   noise_structure = 0) 
...
df3 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-05-04"),]
df4 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-05-11"),]
df5 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-05-18"),]
df6 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-05-25"),]
df7 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-06-01"),]
df8 <- df[df$date>=as.Date("2020-09-23") & df$date<=as.Date("2021-06-08"),]
...
ps_mp_data3 <- list(num_data = nrow(df3),
                    num_path = 4,
                    ...
                    Y = df3$cases,
                    X = df3$t,
                    P = t(matrix(data= c(df3$B.1.177, df3$B.1.1.7, df3$Other, df3$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df3$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit3 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data3)
...
ps_single_data3 <- list(num_data = nrow(df3),
                        num_knots = length(knots3),
                        knots = knots3,
                        spline_degree=3,
                        Y = df3$cases,
...
ps_single_fit3 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
ps_mp_data4 <- list(num_data = nrow(df4),
                    num_path = 4,
                    ...
                    Y = df4$cases,
                    X = df4$t,
                    P = t(matrix(data= c(df4$B.1.177, df4$B.1.1.7, df4$Other, df4$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df4$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit4 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data4)
...
ps_single_data4 <- list(num_data = nrow(df4),
                        num_knots = length(knots4),
                        knots = knots4,
                        spline_degree=3,
                        Y = df4$cases,
...
ps_single_fit4 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
ps_mp_data5 <- list(num_data = nrow(df5),
                    num_path = 4,
                    ...
                    Y = df5$cases,
                    X = df5$t,
                    P = t(matrix(data= c(df5$B.1.177, df5$B.1.1.7, df5$Other, df5$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df5$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit5 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data5)
...
ps_single_data5 <- list(num_data = nrow(df5),
                        num_knots = length(knots5),
                        knots = knots5,
                        spline_degree=3,
                        Y = df5$cases,
...
ps_single_fit5 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
ps_mp_data6 <- list(num_data = nrow(df6),
                    num_path = 4,
                    ...
                    Y = df6$cases,
                    X = df6$t,
                    P = t(matrix(data= c(df6$B.1.177, df6$B.1.1.7, df6$Other, df6$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df6$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit6 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data6)
...
ps_single_data6 <- list(num_data = nrow(df6),
                        num_knots = length(knots6),
                        knots = knots6,
                        spline_degree=3,
                        Y = df6$cases,
...
ps_single_fit6 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
ps_mp_data7 <- list(num_data = nrow(df7),
                    num_path = 4,
                    ...
                    Y = df7$cases,
                    X = df7$t,
                    P = t(matrix(data= c(df7$B.1.177, df7$B.1.1.7, df7$Other, df7$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df7$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit7 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data7)
...
ps_single_data7 <- list(num_data = nrow(df7),
                        num_knots = length(knots7),
                        knots = knots7,
                        spline_degree=3,
                        Y = df7$cases,
...
ps_single_fit7 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
ps_mp_data8 <- list(num_data = nrow(df8),
                    num_path = 4,
                    ...
                    Y = df8$cases,
                    X = df8$t,
                    P = t(matrix(data= c(df8$B.1.177, df8$B.1.1.7, df8$Other, df8$B.1.617.2), ncol=4) ),
                    week_effect = 7,
                    DOW = (df8$t %% 7)+1,
                    cov_structure = 0,
                    noise_structure = 0) 

ps_mp_fit8 <- sampling(ps_mp_mod,
                       iter= 5000,
                       warmup = 1000,
                       chains=4,
                       data = ps_mp_data8)
...
ps_single_data8 <- list(num_data = nrow(df8),
                        num_knots = length(knots8),
                        knots = knots8,
                        spline_degree=3,
                        Y = df8$cases,
...
ps_single_fit8 <- sampling(ps_single_mod,
                           iter= 5000,
                           warmup = 1000,
                           chains=4,
...
# Get incidences
mod_inc <- ps_incidence(ps_mp_fit, df$t, num_days=nrow(df), time_labels = df$date,
                         num_path = 11,
                         pathogen_names = c(lineages_considered,"Other") )

# Get growth rates
mod_gr <- ps_growth_rate(ps_mp_fit, df$t, num_days=nrow(df), time_labels = df$date,
                         num_path = 11,
                         pathogen_names = c(lineages_considered,"Other") )
...
for(i in 1:length( pathogens )){
  ...
  mod_gr[mod_gr$pathogen==pathogens[i] & (mod_gr$time>=min_date & mod_gr$time<=max_date ),]$mask <- 1
  mod_inc[mod_inc$pathogen==pathogens[i] & (mod_inc$time>=min_date & mod_inc$time<=max_date ),]$mask <- 1
  print(pathogens[i])
  #mod_gr<-mod_gr[ (!(mod_gr$pathogen==pathogens[i])) | (mod_gr$time>min_date & mod_gr$time<max_date) ,]
  
}
mod_gr[mod_gr$pathogen=="Other",]$mask <-0
mod_gr[mod_gr$pathogen=="Other" & (mod_gr$time<=as.Date("2021-01-19") | mod_gr$time>=as.Date("2022-03-05")),]$mask <-1

mod_gr$grp <- 0
mod_gr[mod_gr$pathogen=="Other" & mod_gr$time<=as.Date("2021-01-19"),]$grp <-1
...
plt1<-ggplot(mod_inc[mod_inc$pathogen=="Total",])+
  ...
  ylab("Cases")+
  coord_cartesian(xlim=c(min_date+30, max_date-30), ylim=c(0, max(df$cases)))+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b\n%Y")+
  scale_y_continuous(breaks=c(0,50000, 100000, 150000, 200000, 250000))+
  theme_bw(base_size = 14)


plt2<-ggplot(mod_inc[mod_inc$pathogen!="Total" & mod_inc$mask==1,])+
  ...
  coord_cartesian(xlim=c(min_date+30, max_date-30), ylim=c(0, max(mod_inc$y)))+
  scale_x_date(date_breaks = "2 month", date_labels =  "%b\n%Y")+
  scale_y_continuous(breaks=c(0,50000, 100000, 150000, 200000, 250000))+
  ylab("Modelled cases")+
...
plt3<-ggplot(mod_gr[mod_gr$pathogen!="Total" & mod_gr$mask==1,])+
  geom_line(aes(x=time, y=y, color=pathogen, group=interaction(pathogen,grp) ))+
  geom_ribbon(aes(x=time, y=y, ymin=lb_50, ymax=ub_50, fill=pathogen, group=interaction(pathogen,grp)), alpha=0.2)+
  geom_ribbon(aes(x=time, y=y, ymin=lb_95, ymax=ub_95, fill=pathogen, group=interaction(pathogen,grp)), alpha=0.2)+
  theme_bw(base_size = 14)+
...
plt1i <- plt1+coord_cartesian(xlim=c(as.Date("2022-05-01"), max_date),ylim = c(0, 34000))+
...
plt2i <- plt2+coord_cartesian(xlim=c(as.Date("2022-05-01"), max_date),ylim = c(0, 8000))+
  theme(axis.title = element_blank(),
        legend.position = "none",
        plot.background = element_rect(color="black", fill=alpha("grey",0.2)))+
  scale_y_continuous(breaks=c(0,1000,2000,3000,4000, 5000,6000,7000,8000, 10000, 15000))
...
plt1 <- plt1+
  ...
  annotate("rect", xmin = as.Date("2022-05-01"), xmax = max_date+2, ymin = -5000, ymax = 35000, color="black", 
...
plt2 <- plt2+
  ...
  theme(axis.title.x = element_blank(),
                   ...
                   legend.position = "none",
        plot.tag.position = c(0.01,0.96))+
  annotate("rect", xmin = as.Date("2022-05-01"), xmax = max_date+2, ymin = -5000, ymax = 9000, color="black", 
           fill="grey", alpha = 0.2, linetype="dashed")
...
plt1 + plt1i + plt2 +plt2i + plt3 + plot_spacer()+
  plot_layout(ncol = 2, widths = c(3,1), heights = c(2,2,1.5))
...
mod_inc3 <- ps_incidence(ps_mp_fit3, df3$t, num_days=nrow(df3), time_labels = df3$date,
                        num_path = 4,
                        pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
mod_inc4 <- ps_incidence(ps_mp_fit4, df4$t, num_days=nrow(df4), time_labels = df4$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
mod_inc5 <- ps_incidence(ps_mp_fit5, df5$t, num_days=nrow(df5), time_labels = df5$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
mod_inc6 <- ps_incidence(ps_mp_fit6, df6$t, num_days=nrow(df6), time_labels = df6$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
mod_inc7 <- ps_incidence(ps_mp_fit7, df7$t, num_days=nrow(df7), time_labels = df7$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
mod_inc8 <- ps_incidence(ps_mp_fit8, df8$t, num_days=nrow(df8), time_labels = df8$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))


mod_single_inc3 <- ps_single_incidence(ps_single_fit3, df3$t, num_days=nrow(df3), time_labels = df3$date)
mod_single_inc4 <- ps_single_incidence(ps_single_fit4, df4$t, num_days=nrow(df4), time_labels = df4$date)
mod_single_inc5 <- ps_single_incidence(ps_single_fit5, df5$t, num_days=nrow(df5), time_labels = df5$date)
mod_single_inc6 <- ps_single_incidence(ps_single_fit6, df6$t, num_days=nrow(df6), time_labels = df6$date)
mod_single_inc7 <- ps_single_incidence(ps_single_fit7, df7$t, num_days=nrow(df7), time_labels = df7$date)
mod_single_inc8 <- ps_single_incidence(ps_single_fit8, df8$t, num_days=nrow(df8), time_labels = df8$date)


# Get time-varying growth rates
gr_inc3 <- ps_growth_rate(ps_mp_fit3, df3$t, num_days=nrow(df3), time_labels = df3$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
gr_inc4 <- ps_growth_rate(ps_mp_fit4, df4$t, num_days=nrow(df4), time_labels = df4$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
gr_inc5 <- ps_growth_rate(ps_mp_fit5, df5$t, num_days=nrow(df5), time_labels = df5$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
gr_inc6 <- ps_growth_rate(ps_mp_fit6, df6$t, num_days=nrow(df6), time_labels = df6$date,
                         num_path = 4,
                         pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))
gr_inc7 <- ps_growth_rate(ps_mp_fit7, df7$t, num_days=nrow(df7), time_labels = df7$date,
                          num_path = 4,
                          pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))

gr_inc8 <- ps_growth_rate(ps_mp_fit8, df8$t, num_days=nrow(df8), time_labels = df8$date,
                          num_path = 4,
                          pathogen_names = c("B.1.177", "B.1.1.7 (Alpha)", "Other (Wildtype)", "B.1.617.2 (Delta)"))


gr_single_inc3 <- ps_single_growth_rate(ps_single_fit3, df3$t, num_days=nrow(df3), time_labels = df3$date)
gr_single_inc4 <- ps_single_growth_rate(ps_single_fit4, df4$t, num_days=nrow(df4), time_labels = df4$date)
gr_single_inc5 <- ps_single_growth_rate(ps_single_fit5, df5$t, num_days=nrow(df5), time_labels = df5$date)
gr_single_inc6 <- ps_single_growth_rate(ps_single_fit6, df6$t, num_days=nrow(df6), time_labels = df6$date)
gr_single_inc7 <- ps_single_growth_rate(ps_single_fit7, df7$t, num_days=nrow(df7), time_labels = df7$date)
gr_single_inc8 <- ps_single_growth_rate(ps_single_fit8, df8$t, num_days=nrow(df8), time_labels = df8$date)
...
mod_single_inc_comb1 <- rbind(mod_single_inc3,
                              mod_single_inc4, mod_single_inc5, mod_single_inc6,
                              mod_single_inc7, mod_single_inc8)
...
gr_single_comb1 <- rbind(gr_single_inc3, gr_single_inc4,
                         gr_single_inc5, gr_single_inc6,
                         gr_single_inc7, gr_single_inc8)
...
gr_plt <- gr_comb1[gr_comb1$pathogen %in% c("B.1.1.7 (Alpha)", "B.1.617.2 (Delta)", "Total"),]
gr_plt <- gr_plt[gr_plt$group >2,]

gr_single_comb1$pathogen <- "Total\n(variant data not included)"
gr_single_plt <- gr_single_comb1[gr_single_comb1$pathogen %in% c("B.1.1.7 (Alpha)", "B.1.617.2 (Delta)", "Total\n(variant data not included)"),]
...
pltV1<-ggplot(mod_inc_plt)+
  ...
  coord_cartesian(xlim=c(as.Date("2021-04-04"), as.Date("2021-06-28") ), ylim=c(0,18000)  )+
  #geom_line(data=df, aes(x=date, y=cases))+
  ylab("Cases")+
...
gr_plt <- rbind(gr_plt[gr_plt$pathogen=="B.1.617.2 (Delta)" &gr_plt$time >= as.Date("2021-04-28"),],
...
pltV3<-ggplot(gr_plt)+
  ...
  geom_ribbon(data=gr_single_plt, aes(x=time, y=y, ymin=lb_50, ymax=ub_50, fill=pathogen), alpha=0.2)+
  geom_ribbon(data=gr_single_plt, aes(x=time, y=y, ymin=lb_95, ymax=ub_95, fill=pathogen), alpha=0.2)+
  coord_cartesian(xlim=c(as.Date("2021-04-28"), as.Date("2021-06-08") ), ylim=c(-0.15,0.5) )+
  #geom_line(data=df, aes(x=date, y=cases))+
  ylab("Growth rate")+
  geom_hline(yintercept = 0, linetype="dashed")+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b\n%Y")+
...
pltV1<- pltV1 +labs(tag="A") +  theme(legend.position = c(0.25,0.95),
                      legend.background = element_rect(color="black"),
                      plot.tag.position = c(0.0, 0.99))+
  coord_cartesian(xlim=c(as.Date("2021-04-20"), as.Date("2021-06-18") ), ylim=c(0,11000)  )

pltV2 <- pltV2 +labs(tag="B")+ theme(legend.position = c(0.35,0.95),
                                     legend.background = element_rect(color="black"),
...
raw1<-ggplot(data=df_new)+
  ...
  theme(legend.position = "bottom",
        legend.background = element_rect(color="black"))+
  annotate("rect", xmin = as.Date("2021-03-01"), xmax = as.Date("2021-06-28"), ymin = -10, ymax = 100, color="black", 
           fill=NA, alpha = 0.2, linetype = "dashed")+
  labs(tag="A")
...
raw2<-ggplot(data=df_new)+
  geom_col(aes(x=date, y=value, fill=name))+
  theme_bw(base_size = 14)+
  coord_cartesian(xlim=c(as.Date("2021-03-01"), as.Date("2021-06-28")), ylim=c(0,50))+
  scale_x_date(date_breaks = "1 month", date_labels =  "%b\n%Y")+
  ylab("Modelled cases")+
  xlab("Date")+
  scale_fill_manual(values=cols)+
...
ps_growth_rate_advantage <- function(a,
                                     ...
                                     num_days = length(X), time_labels,
                                     days_per_knot = 5, spline_degree = 3,
                                     num_path=4, path_i=c(1,2,3), path_j=c(2,3,4), pathogen_names=c("Influenza A H3N2", "Influenza A H1N1", "Influenza B", "Unknown")){
  ...
  for(i in 2:num_days){
    ...
    for(x in 1:length(path_i) ){
      ...
      quan<- quantile( (a[,j,i] - a[,j,(i-1)])-(a[,k,i] - a[,k,(i-1)]), c(0.5,0.025, 0.25, 0.75, 0.975))
      prop <- length(a[,j,i][ ( (a[,j,i] - a[,j,(i-1)]) - (a[,k,i] - a[,k,(i-1)]) )>0])/length(a[,j,i] - a[,j,(i-1)])
...
ps_a <-  ps_get_a(ps_mp_fit, df$t, num_days=nrow(df), time_labels = df$date, num_path = 11)

# Get growth rate advantage between specified pathogen pairs (path_i vs path_j)
mod_gr_adv <- ps_growth_rate_advantage(ps_a, df$t, num_days=nrow(df), time_labels = df$date,
                                       num_path = 11,
                                       path_i = c(2, 3, 4, 5, 8),
                                       path_j = c(11, 2, 3, 4, 5),
                                       pathogen_names = c(lineages_considered,"Other") )
...
gi_dist_i = c(function(x) gammaDist(b=n/mu_a, n=n, x),
              function(x) gammaDist(b=n/mu_d, n=n, x),
              function(x) gammaDist(b=n/mu_ba1, n=n, x),
              function(x) gammaDist(b=n/mu_ba2, n=n, x),
              function(x) gammaDist(b=n/mu_ba5, n=n, x))

gi_dist_j = c(function(x) gammaDist(b=n/mu_w, n=n, x),
              function(x) gammaDist(b=n/mu_a, n=n, x),
              function(x) gammaDist(b=n/mu_d, n=n, x),
              function(x) gammaDist(b=n/mu_ba1, n=n, x),
              function(x) gammaDist(b=n/mu_ba2, n=n, x))

# Get Rt advantagees assuming same generation interval distribution
mod_Rt_adv <- ps_Rt_advantage(ps_a, df$t, num_days=nrow(df), time_labels = df$date,
                              tau_max = 7,
                              gi_dist = function(x) gammaDist(b=n/mu_w, n=n, x),
                              num_path = 11,
                              path_i = c(2, 3, 4, 5, 8),
                              path_j = c(11, 2, 3, 4, 5),
                              pathogen_names = c(lineages_considered,"Other") )
...
mod_Rt_advVar <- ps_Rt_advantage2(ps_a, df$t, num_days=nrow(df), time_labels = df$date,
                              tau_max = 7,
                              gi_dist_i = gi_dist_i,
                              gi_dist_j = gi_dist_j,
                              num_path = 11,
...
levels(mod_gr_adv$pathogen1) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "BA.5 (Omicron)")
levels(mod_gr_adv$pathogen2) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "Wildtype")

mod_gr_adv$pathogen <- paste(mod_gr_adv$pathogen1, " over ", mod_gr_adv$pathogen2)
...
levels(mod_Rt_adv$pathogen1) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "BA.5 (Omicron)")
levels(mod_Rt_adv$pathogen2) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "Wildtype")

mod_Rt_adv$pathogen <- paste(mod_Rt_adv$pathogen1, " over ", mod_Rt_adv$pathogen2)
...
levels(mod_Rt_advVar$pathogen1) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "BA.5 (Omicron)")
levels(mod_Rt_advVar$pathogen2) <- c("B.1.1.7 (Alpha)",   "B.1.617.2 (Delta)",
                                  "BA.1 (Omicron)",      "BA.2 (Omicron)",
                                  "Wildtype")

mod_Rt_advVar$pathogen <- paste(mod_Rt_advVar$pathogen1, " over ", mod_Rt_advVar$pathogen2)
...
plt_gr2<-ggplot(mod_Rt_adv[mod_Rt_adv$mask==1,])+
  ...
  guides(color=guide_legend(nrow=2,byrow=TRUE))+
  theme(legend.position = "bottom")+
  coord_cartesian(xlim=c(min_date+30, max_date-30),ylim=c(0,4))+
  geom_label(aes(label = "Constant generation interval distribution", x=as.Date("2021-03-01"), y=Inf ), vjust=1.5)
...
plt_gr3<-ggplot(mod_Rt_advVar[mod_Rt_advVar$mask==1,])+
  ...
  guides(color=guide_legend(nrow=2,byrow=TRUE))+
  theme(legend.position = "bottom")+
  coord_cartesian(xlim=c(min_date+30, max_date-30),ylim=c(0,3))+
  geom_label(aes(label = "Generation interval distribution changes between variants", x=as.Date("2021-03-01"), y=Inf ), vjust=1.5)
...
df_gi <- rbind(df_gi1, df_gi2, df_gi3,
               df_gi4, df_gi5, df_gi6)
...
ggplot(df_gi)+
  ...
  geom_vline(data=df_int, aes(xintercept= c(mu_w, mu_a, mu_d, mu_ba1, mu_ba2, mu_ba5), color=Variant ), linetype="dashed")+
  theme_bw(base_size = 14)+
  scale_color_brewer("Variants",palette="Dark2")+
